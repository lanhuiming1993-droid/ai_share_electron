...
def format_evidence_for_hermes(latest_status: dict, evidence: dict, max_evidence_chars: int) -> str:
    ...
    lines.append("- 正文要按主题/产业链拆分，优先呈现可操作的产业线索、矛盾点、催化和风险；不要只写散文式总结。")
    lines.append("- 明确标注信息来自 WeRSS、IMA 知识库或知识星球；微信公众号内容尽量写出具体公众号/作者。")
    lines.append("- 对 IMA live 403/超量这类缓存兜底要如实说明为 cached evidence，不要包装成实时采集成功。")
    lines.append("- 末尾必须有“待核验与风险提示”card，集中列出不确定性最高的事项。")
    lines.append("- 默认交付 PDF：先把完整报告保存为 /tmp/alphadesk-report-{job_id}.html，再调用 render_report_pdf.py --format html 生成 PDF。")
    lines.append("- HTML 示例片段：<div class='card'><p><span class='source-tag source-high'>知识星球</span><span class='source-tag'>WeRSS: 公众号名</span></p><ul><li><span class='fact'>事实</span> ...</li><li><span class='infer'>推断</span> ...</li><li><span class='unverified'>待核验</span> ...</li></ul></div>")

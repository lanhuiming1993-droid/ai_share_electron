...
GROWTH_HUNTER_SKILL_NAME = "a-share-growth-hunter"
GROWTH_HUNTER_SKILL_PATH = HERMES_SKILLS_DIR / GROWTH_HUNTER_SKILL_NAME / "SKILL.md"
...
GROWTH_HUNTER_EXPLICIT_RE = re.compile(
    r"(成长股|戴维斯|双击|黄金击球|成长加速|财务拐点|订单|产能|客户|毛利率|估值弹性|值博率|机构共识|催化)",
    re.I,
)
GROWTH_HUNTER_BROAD_RE = re.compile(
    r"(板块|行业|产业链|市场|大盘|指数|宏观|政策|信源|三信源|聚合|报告|资讯|复盘)",
    re.I,
)
...
def _looks_like_single_stock_query(query: str) -> bool:
    text = re.sub(r"\s+", "", str(query or "")).strip()
    if not text:
        return False
    if re.search(r"\b[036]\d{5}\b", text):
        return True
    if GROWTH_HUNTER_EXPLICIT_RE.search(text):
        return True
    if GROWTH_HUNTER_BROAD_RE.search(text):
        return False
    return 2 <= len(text) <= 12 and bool(re.search(r"[\u4e00-\u9fff]", text))


def _load_growth_hunter_skill(max_chars: int = 7000) -> str:
    try:
        text = GROWTH_HUNTER_SKILL_PATH.read_text(encoding="utf-8").strip()
    except OSError:
        return ""
    if len(text) <= max_chars:
        return text
    return text[:max_chars].rstrip() + f"\n\n[Growth hunter framework truncated: {len(text) - max_chars} chars]"


def _growth_hunter_prompt_section(query: str) -> str:
    if not _looks_like_single_stock_query(query):
        return ""
    framework = _load_growth_hunter_skill()
    if not framework:
        return ""
    return f"""

# A-Share Growth Hunter Framework

The user query appears to be a company-level A-share growth-stock analysis target: {query}

Use the following framework as an analysis structure only. It is not a data source. All facts must still come from AlphaDesk evidence and external skill evidence.
...
def _analysis_prompt(evidence_text: str, *, days: int, query: str) -> str:
    ...
    growth_section = _growth_hunter_prompt_section(query)
    growth_requirement = (
        "\n12. 如果出现 A-Share Growth Hunter Framework，必须增加“市值区间与六维评分”“公开披露与私域线索一致性”“右侧确认信号/证伪信号”三个部分；如果证据不足，不得强行给高确信结论。"
        if growth_section
        else ""
    )

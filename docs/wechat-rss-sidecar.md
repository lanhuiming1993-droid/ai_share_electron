...
本地托管容器默认启用正文抓取、全文 RSS 和每 15 分钟一次的缺失正文补抓。WeRSS 会先同步公众号文章索引，再在受控频率下补充正文；正文尚未回填时，RSS 中可能暂时只有标题、摘要和原文链接。AlphaDesk 按小页读取全文 RSS，单页保留 8 MB 防护，并在越过任务时间窗后停止翻页，避免全文积累后出现大响应失败或无边界抓取。
